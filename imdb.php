...
//release info
preg_match_all('/title\/.*?\/\?ref_=adv_li_tt/',$result,$match);

for($i=0;$i<count($match[0]);$i++) {
    $releaseinfo = get_release_info($match[0][$i]);
    
    if ($releaseinfo != NULL) {
        $movies['release_info'][$i] = $releaseinfo;
        if ($releaseinfo < date("Y-m-d")) {
            $movies['status'][$i] = "Released";
        } else {
            $movies['status'][$i] = "Not Released";
        }
    } else {
        $movies['release_info'][$i] = '';
        $movies['status'][$i] = "Not Released";
    }
}
